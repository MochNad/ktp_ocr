...
def main(image):
    ...
    nama_match = re.search(r"Nama - (.+)", result)
    if nama_match:
        nama = nama_match.group(1)

    ttl_match = re.search(r"TempatTgi Lahir (.+)", result)
    if ttl_match:
        tempatTglLahir = ttl_match.group(1).strip()

    jk_match = re.search(r"Jenis kelamun :(.+)", result)
    if jk_match:
        jenisKelamin = jk_match.group(1).strip()
    
    gd_match = re.search(r"Gol. Darah '(.+)", result)
    if gd_match:
    ...
    rtrw_match = re.search(r"RTRW : (.+)", result)
    if rtrw_match:
        rtRw = rtrw_match.group(1).strip()

    kel_desa_match = re.search(r"KeWWesa : (.+)", result)
    if kel_desa_match:
        kelDesa = kel_desa_match.group(1).strip()

    kecamatan_match = re.search(r"Kecamatan — :(.+)", result)
    if kecamatan_match:
        kecamatan = kecamatan_match.group(1).strip()

    agama_match = re.search(r"Agama “ (.+)", result)
    if agama_match:
        agama = agama_match.group(1).strip()

    status_kawin_match = re.search(r"Status Perkawmnan . (.+)", result)
    if status_kawin_match:
        statusPerkawinan = status_kawin_match.group(1).strip()

    pekerjaan_match = re.search(r"Pekerjaan (.+)", result)
    if pekerjaan_match:
        pekerjaan = pekerjaan_match.group(1).strip()

    kewarganegaraan_match = re.search(r"Kewarganegaraan . (.+)", result)
    if kewarganegaraan_match:
        kewarganegaraan = kewarganegaraan_match.group(1).strip()

    berlaku_hingga_match = re.search(r"Berlaku Hingga (.+)", result)
